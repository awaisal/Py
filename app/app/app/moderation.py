...
# in-memory spam tracking (fast). strikes are persisted in sqlite.
...
def _punishment_for_strike(strike: int) -> Tuple[str, int]:
    # returns (action, duration_sec)
    if strike == 1:
        return ("restrict", 5 * 24 * 3600)      # 5 days
    if strike == 2:
        return ("restrict", 30 * 24 * 3600)     # ~1 month (30 days)
...
async def apply_punishment(update: Update, context: ContextTypes.DEFAULT_TYPE, reason: str):
    ...
    # Don't punish admins
    if _is_admin(context, chat_id, user_id):
        return
